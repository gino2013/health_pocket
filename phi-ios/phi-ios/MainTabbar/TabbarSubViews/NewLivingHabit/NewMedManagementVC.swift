...
class NewMedManagementVC: BaseViewController, FSCalendarDelegate, FSCalendarDataSource, FSCalendarDelegateAppearance {
    ...
    private let sectionIdentifier = "ReminderTimeTVCell"
    ...
    func updateUI() {
        tblView.register(nibWithCellClass: ReminderTimeTVCell.self)
    ...
    func pushToAddMedicinePage() {
        ...
        /*
        let storyboard = UIStoryboard(name: "AddReminder", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "NewManualStep1VC") as! NewManualStep1VC
        ...
        */
        ...
        let vc = storyboard.instantiateViewController(withIdentifier: "NewSetttStep1VC") as! NewSetttStep1VC
        vc.delegate_MedicationManagementVC = self
        SharingManager.sharedInstance.currentSetReminderMode = .manual
        vc.hidesBottomBarWhenPushed = true
        self.navigationController?.pushViewController(vc, animated: true)
...
extension NewMedManagementVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            guard let reminderSection = tableView.dequeueReusableCell(withIdentifier: sectionIdentifier, for: indexPath) as? ReminderTimeTVCell else {
                fatalError("Issue dequeuing \(sectionIdentifier)")
            }
            
            if let sectionExts = sectionExtDict[dateFormatter.string(from: selectedDate!)] {
                reminderSection.configureCell(title: sectionExts[indexPath.section].title)
...
extension NewMedManagementVC: NewSetttStep2VCDelegate {
...
extension NewMedManagementVC: ReminderTViewCellDelegate {
...
extension NewMedManagementVC: MedicineTakingSettingVCDelegate {
...
extension NewMedManagementVC: EditSingleReminderViewControllerDelegate {
